function meds_main_glider_data_processing_rt()
input_deployment_file = 'w:\glider_project\gliders_canada_deployments.csv';
input_meta_file = 'w:\glider_project\gliders_canada_meta.csv';
...
% move down to according to glider mission
...
%% Check the input folder for glider to see if there is any data to process
if ispc
    roote='w:\glider_project\';
else
    roote='/u01/rapps/glider_project/';
end
seaExplorer_folder = dir (fullfile(roote, 'data', 'seaexplorer', 'input'));


glider_type_mission={};
for i = 1:length(seaExplorer_folder)
    file_name = {seaExplorer_folder(i).name};    
    if (~strcmpi(file_name, '.') && ~strcmpi(file_name, '..'))        
        s = strsplit(char(file_name), '.');    
        s2 = [s{1} '.' s{2}];
        if (isempty(find(strcmpi(glider_type_mission, s2))))
            glider_type_mission(end +1,:)=  {s2};           
        end 
    end
   
end
slocum_folder = dir (fullfile(roote, 'data', 'slocum', 'binary'));
...
deployment_list = readcsv_generic(input_deployment_file);
gliders_meta = readcsv_generic (input_meta_file);
deployments =struct([]);
for j = 1: numel(glider_type_mission)
    mission = char(glider_type_mission(j));
    display(mission(j));
    s = strsplit(mission,'.');
    glider_name = char(s{1});
    glider_mission = char(s{2}); 
    for idx = 1: numel(deployment_list)
        if (strcmpi(deployment_list(idx).glider_name, glider_name)...
                && strcmpi(deployment_list(idx).deployment_id, glider_mission)...
                && strcmpi(deployment_list(idx).active, 'Y'))                        
            for m_idx = 1: numel(gliders_meta)
                if (strcmpi(gliders_meta(m_idx).glider_name, glider_name))
                    display ([glider_name ' meta glider name: ' gliders_meta(m_idx).glider_name])
                    meta = gliders_meta(m_idx);
                    f = fieldnames(meta);
                    for k = 1:length(f)
                        deployment_list(idx).(f{k}) = meta.(f{k});
                    end
                    deployments{end+1}=[deployment_list(idx)];
                end
            end    
        end
...
display (['Number of deployment to process ' numel(deployments)]);
%% Process active deployments.
for deployment_idx = 1:numel(deployments)
  ...
  [config.figures_processed, config.figures_gridded] = configFigures(deployment);
  config.file_options_seaexplorer = configRTFileOptionsSeaExplorer(deployment);
  source_files = {};
  meta_raw = struct();
  data_raw = struct();
  ...
  deployment_start = deployment.deployment_start_date;
  deployment_end = deployment.deployment_end_date;
...
display (['working on glider type ' glider_type]);
  switch glider_type
    case 'slocum_g1'
 ...
 %% Configure netCDF variables for QC variables.
  ...
  if strcmpi(deployment_start, 'NaN')
      download_start = datenum(datestr(now, 'yyyy-mm-dd'), 'yyyy-mm-dd')
  else
  ...
  %% sorting file based on date
      [~,idx]=sort([new_files.datenum]);
      ...
      switch glider_type
        ...
        case {'seaexplorer'}
          outputs.netcdf_l0 = generateOutputNetCDF( ...
              ...
              'position', {'Lon' 'Lat';'NAV_LONGITUDE' 'NAV_LATITUDE' }, ...             
              'position_conversion', @nmea2deg, ...
              'vertical',            {'Depth' 'NAV_DEPTH' }, ...
              'vertical_conversion', {[]        @(z)(z * 10)}, ...
              'vertical_positive',   {'down'} );      
      end
...
if exist(netcdf_l1_file, 'file')
    command = ['java' ' -jar' ...
            ' -Dlog4j.configuration=file:///"C:\Users\trana\java projects\config\log4j.properties"' ...
            ' "C:\Users\trana\java Projects\J2SE\GliderNetcdf\SocibNCToCoriolisNC\socibNCToCoriolisNC.jar" '...
            netcdf_l1_file...
    ...
    wmo_number_filename = [deployment.wmo_platform_code '_'...
        datestr(deployment_start, 'yyyymmdd')];
     meds_gl_generate_prof_from_socib_ego_file('egofile',realtime_ncfile,'wmo',...
     ...
     [pathstr,name, ext]= fileparts (realtime_ncfile);
         mono_nc_dir = [public_folder filesep name filesep '*.nc'];
        files = ls (mono_nc_dir);
        for i = 1: size (files, 1)
            input = [public_folder filesep name filesep files(i,:)];
            fprintf('Processing file : %s\n', input);
            gl_add_rtqc_to_profile_file(input)
        end
end
%% send plot and netcdf file to ftp site
% ftp_command = ['"C:\Program Files (x86)\CoreFTP\coreftp.exe"' ...
%         ' -s -O -site trana_ftp_isdm_gc_ca -u '...
%         ' "W:\glider_project\data\public\*.png"'...
%         ' *'...        
%         ];
% [ftpstatus, cmdout] = system(ftp_command);
% ftp_command = ['"C:\Program Files (x86)\CoreFTP\coreftp.exe"' ...
%         ' -s -O -site trana_ftp_isdm_gc_ca -u '...
%         ' "W:\glider_project\data\public\*.nc"'...
%         ' *'...        
%         ];
% [ftpstatus, cmdout] = system(ftp_command);
